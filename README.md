...
与仅输出关键词的传统打标器（如 WD14）不同，TagMaster 利用 GPT-4o、Gemini 1.5、DeepSeek 等现代**视觉语言模型 (VLM)**，生成详尽的自然语言描述或高质量标签列表。这对于训练 SDXL、Pony 和 Flux 等新一代模型至关重要。
...
* **Batch Rename**: Rename images sequentially (e.g., my\_lora\_001.png) while keeping files synced on disk.  
...
* **批量重命名**: 按序重命名图片（如 my\_lora\_001.png），并同步更新本地文件。  
